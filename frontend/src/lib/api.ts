...
export interface ProjectFileFilters {
  ...
  category?: string;
  species_id?: string;
  // Smart filters that match against source_metadata (incl. inferred).
  gene?: string;
  meta_kind?: string;
  tool?: string;
  species_ids?: string[];
...
export async function listProjectFiles(
  ...
  if (filters.meta_kind) params.set('meta_kind', filters.meta_kind);
  if (filters.tool) params.set('tool', filters.tool);
  if (filters.species_ids?.length) params.set('species_ids', filters.species_ids.join(','));
  if (filters.name_substring) params.set('name_substring', filters.name_substring);
  const qs = params.toString();
  return api(`/api/projects/${slug}/files${qs ? `?${qs}` : ''}`);
}

export interface ProjectFileFacets {
  genes: string[];
  kinds: string[];
  tools: string[];
  categories: string[];
  species: Array<{ id: string; name: string; common?: string; code: string }>;
  n_files: number;
}

export async function getProjectFileFacets(slug: string): Promise<ProjectFileFacets> {
  return api(`/api/projects/${slug}/files/facets`);
}
...
export async function renameProjectFile(
  slug: string, fileId: string, newName: string
): Promise<ProjectFile> {
  return api(`/api/projects/${slug}/files/${fileId}`, {
    method: 'PATCH',
...
export async function bulkDownloadFiles(
  ...
  // Hits the streaming endpoint with credentials, then triggers a browser save.
  const supabase = supabaseBrowser();
  const { data: { session } } = await supabase.auth.getSession();
  if (!session?.access_token) throw new Error('not authenticated');

  const res = await fetch(`/api/projects/${slug}/files/bulk-download`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${session.access_token}`,
    },
    body: JSON.stringify({ file_ids: fileIds, format }),
  });
  if (!res.ok) {
    const errText = await res.text().catch(() => '');
    throw new Error(`bulk download failed: ${res.status} ${errText}`);
  }
  const blob = await res.blob();
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = `${slug}_files.${format}`;
  document.body.appendChild(a);
  a.click();
  document.body.removeChild(a);
  // Slight delay before revoking so the download has time to start
  setTimeout(() => URL.revokeObjectURL(url), 1000);
}

export async function renameRun(
  slug: string, runId: string, newLabel: string
): Promise<any> {
...
export interface GeneFetchResult {
  saved_files: ProjectFile[];
  warnings: string[];
  gene?: string;
  taxid?: number;
  n_saved?: number;
  selected_accession?: string;
...
export async function fetchNcbiGeneToProject(
  ...
  args: {
    ...
    species_id?: string;
    save_mrna?: boolean;
    save_genomic_region?: boolean;
    genomic_flank_bp?: number;
    extract_cds?: boolean;
    save_protein?: boolean;
    save_genbank?: boolean;
  }
): Promise<GeneFetchResult> {
...
export async function listProjectMembers(slug: string): Promise<ProjectMember[]> {
  return api<ProjectMember[]>(`/api/projects/${slug}/members`);
}

export async function inviteProjectMember(
  slug: string,
  args: { email: string; role: 'editor' | 'viewer' }
): Promise<ProjectMember> {
  return api(`/api/projects/${slug}/members`, {
    method: 'POST',
    body: JSON.stringify(args),
  });
}
...
export async function updateProjectMemberRole(
  ...
  return api(`/api/projects/${slug}/members/${memberId}`, {
    method: 'PATCH',
    body: JSON.stringify({ role }),
  });
}

export async function removeProjectMember(slug: string, memberId: string): Promise<void> {
  await api(`/api/projects/${slug}/members/${memberId}`, { method: 'DELETE' });
}
...
export async function adminListJobs(limit = 200): Promise<Job[]> {
  return api<Job[]>(`/api/admin/jobs?limit=${limit}`);
}
...
export interface Assembly {
  ...
  size_mb: number | null;
  ftp_url: string;
  type: 'GCF' | 'GCA' | 'NUC' | 'other';
  source_db?: string; // 'nuccore' for viral/organelle genome records
  is_refseq?: boolean;
}
...
export async function retryToolRun(slug: string, runId: string): Promise<ToolRun> {
  return api(`/api/projects/${slug}/runs/${runId}/retry`, { method: 'POST' });
}
